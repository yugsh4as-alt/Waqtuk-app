...
public class AppPreferences {
    ...
    private static final String KEY_TODAY_TIMES = "today_times";

    public void saveTodayTimes(double[] times) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < times.length; i++) {
            if (i > 0) sb.append(",");
            sb.append(times[i]);
        }
        prefs.edit().putString(KEY_TODAY_TIMES, sb.toString()).apply();
    }

    public double[] getTodayTimes() {
        String raw = prefs.getString(KEY_TODAY_TIMES, null);
    ...
    private static final String KEY_LAST_QURAN_PAGE  = "last_quran_page";
    private static final String KEY_QURAN_BOOKMARK   = "quran_bookmark";

    public void setLastQuranPage(int page) {
        prefs.edit().putInt(KEY_LAST_QURAN_PAGE, page).apply();
    }

    public int getLastQuranPage() {
        return prefs.getInt(KEY_LAST_QURAN_PAGE, 0);
    }

    public void setQuranBookmark(int page) {
        prefs.edit().putInt(KEY_QURAN_BOOKMARK, page).apply();
    }

    public int getQuranBookmark() {
        return prefs.getInt(KEY_QURAN_BOOKMARK, 0);
    }


    private static final String KEY_QURAN_PAGE     = "quran_page";
    private static final String KEY_QURAN_BOOKMARK = "quran_bookmark";

    public void setLastQuranPage(int page) {
        prefs.edit().putInt(KEY_QURAN_PAGE, page).apply();
    }

    public int getLastQuranPage() {
        return prefs.getInt(KEY_QURAN_PAGE, 0);
    }
